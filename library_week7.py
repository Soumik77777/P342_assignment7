...
def create_csv(name,xlist,ylist):                     #name is the name of file
    with open(name+".csv", 'w', newline='') as file:
        writer = csv.writer(file)
        for i in range(len(xlist)):
            writer.writerow([xlist[i], ylist[i]])

#Week7_Differential Equations---------------------------------------------------
...
#Shooting Method
def shooting(f1,f2,yn):

    def RungeKutta(zh,zt):
        (X1, Y1) = RK4(0, 1, zh, 0.01, 1, f1, f2)
        (X2, Y2) = RK4(0, 1, zt, 0.01, 1, f1, f2)
        return X1,Y1,X2,Y2
    
    def estimate():
        for i in range(20):
            estimate1 = input("Input first estimation:")
            zh = float(estimate1)
            estimate2 = input("Input second estimation:")
            zt = float(estimate2)
    ...
    for i in range(20):
        ...
        if Y1[99]>yn and Y2[99]<yn:
            ...
            Y3,X3=RK4(0, 1, z, 0.01, 1, f1, f2)
            if abs(Y3[99]-yn)<0.02:
                print("The obtained value of z=",z)
                return Y3[99],Y3,X3
            elif z<yn:
                zt=z
            else:
